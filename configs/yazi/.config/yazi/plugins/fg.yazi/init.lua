...
local function entry(_, args)
	...
	local preview_cmd =
		[===[line={2} && begin=$( if [[ $line -lt 7 ]]; then echo $((line-1)); else echo 6; fi ) && bat --highlight-line={2} --color=always {1}]===]
	if shell_value == "fish" then
		preview_cmd =
			[[set line {2} && set begin ( test $line -lt 7  &&  echo (math "$line-1") || echo  6 ) && bat --highlight-line={2} --color=always {1}]]
	elseif shell_value == "nu" then
		preview_cmd =
			[[let line = ({2} | into int); let begin = if $line < 7 { $line - 1 } else { 6 }; bat --highlight-line={2} --color=always {1}]]
	end
	...
	elseif args[1] == "rga" and (shell_value == "bash" or shell_value == "zsh") then
		...
		cmd_args = [[rg --color=always --line-number --no-heading --smart-case '' | fzf-tmux -p 80% --ansi --preview=']]
			.. preview_cmd
			.. [[' --delimiter=':' --preview-window='right:60%' --nth='3..']]
	end
